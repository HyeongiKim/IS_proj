...
void aes_encr(unsigned char *state, unsigned char *expandedKey, int flag)
{
...
/* Print unsigned char* sen */
void ans_printf(unsigned char *sen)
{
	int i;
	for(i=0; i<16;i++){
		printf("%02x ",sen[i]);
...
int ham_dist(unsigned char* rhs, unsigned char* lhs)
{
	...
	for (index=0; index<16; index++){
		cmp = rhs[index]^lhs[index];
		for (bcur=0; bcur<8; bcur++){
			if((cmp>>bcur)&1)
				count++;
		}
	}
...
{
	int round;
	printf("Hamming distance between 2CT.\n");
	for (round=0;round<10;round++){
		printf("	Round%d	: %d\n", round+1, ham_dist(ct[round],ct_diff[round]));
	}
}
...
int main( void )
{
	...
	unsigned char state2[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
							0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff};
	unsigned char test[16];
	
	/* CASE 1 */
	unsigned char zeroPT[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
							0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	unsigned char zeroKey[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
							0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	unsigned char changed_PT[16];
	...
	/* Encrypt */
	printf("E(p,k)-> CT	: ");
	aes_encrypt(zeroPT,zeroKey,0);
	ans_printf(zeroPT);

	/* Decrypt */
	printf("D(c,k)-> PT	: ");
	aes_decrypt(zeroPT,zeroKey);
	ans_printf(zeroPT);
	...
	ans_printf(zeroPT);
	for(int i=0;i<16;i++){
		changed_PT[i] = zeroPT[i];
	}
	/* Change 1 bit */
	if(!(changed_PT[0]&1))
		changed_PT[0] = changed_PT[0]+1;
	else
		changed_PT[0] = changed_PT[0]&0xFE;
	printf("changed_PT	: ");
	ans_printf(changed_PT);
	aes_encrypt(changed_PT,zeroKey,1);
	print_ham_dist();
	
	printf(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n\n");

	/* CASE 2 */
	unsigned char ranPT[16];
	unsigned char ranKey[16];
	unsigned char changed_PT2[16];
	rand_bit(ranPT);
	rand_bit2(ranKey);
	...
	/* Encrypt */
	printf("E(p,k)-> CT	: ");
	aes_encrypt(ranPT,ranKey,0);
	ans_printf(ranPT);

	/* Decrypt */
	printf("D(c,k)-> PT	: ");
	aes_decrypt(ranPT,ranKey);
	ans_printf(ranPT);
	/* 2-(ii) ***********************************/
	printf("\n2-(ii)>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
	printf("random PT	: ");
	...
	/* Copy ranPT */
	for(int i=0;i<16;i++){
		changed_PT2[i] = ranPT[i];
	}
	
	/* Change 1 bit */
	if(!(changed_PT2[0]&1))
		changed_PT2[0] = changed_PT2[0]+1;
	else
		changed_PT2[0] = changed_PT2[0]&0xFE;
	printf("changed_PT	: ");
	ans_printf(changed_PT2);
	aes_encrypt(changed_PT2,ranKey,1);
	print_ham_dist();
	printf(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n\n");
